...
function sendPassword() {
  const man1Password = parseInt(password.value, 10);
  const man1PasswordLength = Number(password.value.length);
  const sendPassword = JSON.stringify({ man1Password, man1PasswordLength });
  if (man1PasswordLength < 4 || isError.value || isSecondError.value) return;
